...
begin
   ...
   h1 := dbms_datapump.open('EXPORT','TABLESPACE');
   
   -- Specify or add a dump file of the dump file set, and a log file:
   dbms_datapump.add_file(h1,'expdp_nolog_meta.dmp','NOLOGMETA2',NULL,DBMS_DATAPUMP.KU$_FILE_TYPE_DUMP_FILE,1);
   dbms_datapump.add_file(h1,'expdp_nolog_meta.log','NOLOGMETA2',NULL,DBMS_DATAPUMP.KU$_FILE_TYPE_LOG_FILE);
   
   -- Specify which items to process:
   dbms_datapump.metadata_filter(h1,'TABLESPACE_EXPR','= ''TBLSP_NOLOGTBL''');
