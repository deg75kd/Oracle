...
begin
   ...
   --dbms_datapump.metadata_filter(h1,'SCHEMA_EXPR','IN (select distinct owner from dba_tables where logging=''NO'' and TEMPORARY=''N'' and dropped=''NO'')'); 
   --dbms_datapump.metadata_filter(h1,'SCHEMA_EXPR','IN (''AUDIT_GKPR'',''DBSNMP'',''GGTEST'',''SYS'')'); 
   --dbms_datapump.metadata_filter(h1,'SCHEMA_LIST','''AUDIT_GKPR'',''DBSNMP'',''GGTEST'',''SYS'''); 
   --dbms_datapump.metadata_filter(h1,'SCHEMA_LIST','''AUDIT_GKPR'',''GGTEST''');
   ...
   --dbms_datapump.metadata_filter(h1,'NAME_EXPR','IN (select table_name from dba_tables where logging=''NO'' and TEMPORARY=''N'' and dropped=''NO'')','TABLE');
   --dbms_datapump.metadata_filter(h1,'NAME_LIST','''DB_INFO'',''EAPP_T_PDFTEMPLATES'',''EMPLOYEE'',''MASTER'',''OEM_JOB_DATE'',''PROFILE_STANDARDS'',''PROFILE_TEMP'',''PROFILE_USERS'',''SQL_RESULTS_1'',''SQL_RESULTS_2'',''SQL_RESULTS_3'',''SQL_TUNE_OEM_STAGING'',''SQL_TUNE_STAGING'',''SQL_TUNE_STAGING_BKP'',''SQL_TUNE_TASK_STAGING'',''STATS_USER_LOG'',''SYSAUX_MONITOR'',''MGMT_SNAPSHOT'',''MGMT_SNAPSHOT_SQL'',''OBJECT_COUNT_AIX'',''ROW_COUNT_AIX'',''WRR$_REPLAY_COMMITS'',''WRR$_REPLAY_DATA'',''WRR$_REPLAY_DEP_GRAPH'',''WRR$_REPLAY_REFERENCES'',''WRR$_REPLAY_SEQ_DATA''','TABLE');
   --dbms_datapump.metadata_filter(h1,'NAME_LIST','''DB_INFO'',''OBJECT_COUNT_AIX''','TABLE');
